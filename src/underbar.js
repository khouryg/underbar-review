(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    for (let i = 0; i < array.length; i++) {
      if (iterator !== undefined && !iteratorCache.includes(iterator(array[i]))) {
        cache.push(array[i]);
        iteratorCache.push(iterator(array[i]));
      } else if (!cache.includes(array[i]) && !iterator) {
        cache.push(array[i]);
      }
    }
    return cache;
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(acc, curr) {
      return Boolean(iterator(curr) && acc);
    }, true);
  };
  ...
  _.memoize = function(func) {
    ...
    return function() {
      ...
      if (output[args] === undefined) {
        return (output[args] = func.apply(this, arguments));
      }
    };
  };
